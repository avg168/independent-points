...
(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // V1.3.2 does not depend on replacing Trust Wallet's injected provider.
    // app-loader-v1.3.2.js routes the existing app.js directly to IPTWalletConnectBridge.
    installWalletSwitchButton();

    if (footer) {
      footer.textContent = footer.textContent
        .replace("V1.3", "V1.3.2")
        .replace("交易防重複 V1.3 · 一鍵切換錢包", "交易防重複 V1.3.2 · 一鍵切換錢包");
    }
    ...
    if (status && status.textContent.includes("合約已部署")) {
      status.textContent =
        "WalletConnect Bridge V1.3.2.2 已就緒；可連接 Trust Wallet 並讀取合約。";
    }
  });
})();
