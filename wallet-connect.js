...
function setBusy(isBusy) {
  ...
  el("walletConnectBtn").disabled = isBusy;
  el("switchSepoliaBtn").disabled = isBusy || !window.ethereum;
...
async function connectWalletOnly() {
  if (!window.ethereum) throw Error("未偵測到錢包。請在 MetaMask App 內建瀏覽器中開啟本網站。");
  if (!window.ethers) throw Error("錢包程式庫載入失敗。請檢查網路連線後重新整理網頁。");
  ...
  el("copyWalletBtn").disabled = false;
  if (network.chainId !== CHAIN_ID) {
    el("chainLabel").textContent = `目前 Chain ID ${network.chainId}（非 Sepolia）`;
    show("錢包已連接，但目前不是 Sepolia。請按「切換至 Sepolia」並在 MetaMask 確認；不需要充值。", true);
    return;
  }
  el("chainLabel").textContent = "Sepolia（11155111）";
  show("錢包連接成功，已在 Sepolia 測試網。尚未部署 IPT 合約時，無需填寫合約地址，也無法轉移鏈上 IPT 點數。");
}
async function connect() {
  ...
  if (!el("contractAddress").value.trim()) throw Error("尚未部署 IPT 合約，請先按「只連接 MetaMask 錢包」。部署合約後，才能填入合約地址並讀取點數。");
...
el("walletConnectBtn").addEventListener("click", async () => {
  setBusy(true);
  show("正在請求 MetaMask 連線，請查看錢包授權畫面…");
  try { await connectWalletOnly(); }
  catch (err) { show(errorText(err), true); }
  finally { setBusy(false); }
});
el("switchSepoliaBtn").addEventListener("click", async () => {
  setBusy(true);
  show("請在 MetaMask 中確認切換至 Sepolia…");
  try {
    if (!window.ethereum) throw Error("請先使用 MetaMask 內建瀏覽器開啟網站。");
